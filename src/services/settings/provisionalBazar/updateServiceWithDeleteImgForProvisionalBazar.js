...
const updateServiceWithDeleteImgForProvisionalBazar = async (
  Request,
  DataModel
) => {
  ...
  try {
    ...
    if (deleteImg.result === "ok") {
      var pullImg = await DataModel.findOneAndUpdate(
        ...
        { $pull: { provisionalBazar: { public_id: imgId } } }
      );
...
module.exports = updateServiceWithDeleteImgForProvisionalBazar;
