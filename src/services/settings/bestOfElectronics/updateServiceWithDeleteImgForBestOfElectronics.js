...
const updateServiceWithDeleteImgForBestOfElectronics = async (
  Request,
  DataModel
) => {
  ...
  try {
    ...
    if (deleteImg.result === "ok") {
      var pullImg = await DataModel.findOneAndUpdate(
        ...
        { $pull: { bestOfElectronics: { public_id: imgId } } }
      );
...
module.exports = updateServiceWithDeleteImgForBestOfElectronics;
