...
const updateServiceWithDeleteImgForDealerBrandLogo = async (
  Request,
  DataModel
) => {
  ...
  try {
    ...
    if (deleteImg.result === "ok") {
      var pullImg = await DataModel.findOneAndUpdate(
        ...
        { $pull: { dealerBrandLogo: { public_id: imgId } } }
      );
...
module.exports = updateServiceWithDeleteImgForDealerBrandLogo;
